...
export default function AlterarPerfil() {
  ...
  const handleAlterarOrganizador = async (e) => {
    e.preventDefault();

    const data = new FormData();

    if (nome === "") {
      data.append("nome", organizador.nome);
    } else {
      data.append("nome", nome);
    }
    if (telefone === "") {
      data.append("telefone", organizador.telefone);
    } else {
      data.append("telefone", telefone);
    }
    if (whatsapp === "") {
      data.append("whatsapp", organizador.whatsapp);
    } else {
      data.append("whatsapp", `+55 ${whatsapp}`);
    }
    if (cnpj === "") {
      data.append("cnpj", organizador.cnpj);
    } else {
      data.append("cnpj", cnpj);
    }
    if (rua === "") {
      data.append("rua", organizador.rua);
    } else {
      data.append("rua", rua);
    }
    if (numero === "") {
      data.append("numero", organizador.numero);
    } else {
      data.append("numero", numero);
    }
    if (cidade === "") {
      data.append("cidade", organizador.cidade);
    } else {
      data.append("cidade", cidade);
    }
    if (uf === "") {
      data.append("uf", organizador.uf);
    } else {
      data.append("uf", uf);
    }
    ...
    try {
      ...
      Swal.fire({
        ...
        title: "Oops...",
        text:
          "Não foi possivel editar esse perfil.\nTente novamente mais tarde.",
